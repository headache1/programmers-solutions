...
```text
과거 풀이 여부: 사용자가 전에 풀어본 문제라고 확인
GitHub 확인: origin/main에서 42577 Java 풀이를 찾지 못함
모드: 기존 코드 없는 재현성 진단
등급 기준: 35분 이내 A, 50분 이내 B
힌트: H0
기존 코드·해설·질문 게시판: 열람 금지
Java API 검색: 가능하지만 사용하면 J 기록
목표: 예전 풀이를 보지 않고 현재 기억과 설계로 통과
중단 기준: 15분에 방향 점검, 35분에 불변식 점검, 50분에 학습 전환
```
