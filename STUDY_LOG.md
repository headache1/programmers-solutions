...
```text
GitHub 확인: origin/main에서 42578 Java 풀이를 찾지 못함
과거 풀이 여부: 사용자가 직접 처음 풂 / 전에 풀어봄 / 기억 안 남 중 표시
모드: 과거 풀이 여부에 따라 신규 학습 또는 재현성 진단
힌트: H0
이전 코드·해설·질문 게시판: 열람 금지
Java API 검색: 가능하지만 사용하면 J 기록
전에 풀어봄: 35분 A 기준, 50분 B 기준
처음 풂·기억 안 남: 최대 50분 후 힌트 사다리 적용
```
...
작은 조합표:
종류가 1개일 때 가능한 선택 수:
종류가 2개이고 각 개수가 a, b일 때 가능한 선택 구조:
...
확인할 예외:
종류가 하나뿐 / 각 종류에 옷이 하나씩 /
한 종류에 여러 옷 / 아무것도 입지 않는 선택의 처리
```
