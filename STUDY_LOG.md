...
목표일: 2026-09-01
기준 언어: Java
핵심 완료 기준: 제출 통과에 그치지 않고 새 문제에서 제약 → 상태 → 자료구조를 다시 선택할 수 있는가
...
6. ~~정답 구조나 코드를 본 문제는 반드시 코드를 닫고 D0 빈 화면 재구현을 한다.~~ → C는 48시간 안에 구조를 20분 복원하고, D/E만 가장 가까운 일요일 버퍼에서 40분 한 번 재시도한다.
7. ~~복습은 D0 / D1 / D3 / D7 일정으로 진행한다.~~ → A는 후속 재코딩 없음, B는 다음 날 5분 구두 회상만 진행한다.
8. 문제 수나 같은 답의 재암기보다 새 문제에서 패턴을 선택하는 능력을 진도 기준으로 삼는다.
...
12. ~~실제 날짜가 계획 날짜보다 밀린 동안에는 하루에 계획표 2일치를 진행한다.~~ → 8/1~8/3은 미진행으로 보존하고 소급하지 않는다. 8/4부터 실제 날짜에 배정된 하루 한 문제만 수행한다.
13. 일요일은 D/E가 있을 때만 한 문제를 40분 재시도하는 버퍼일이며, 대상이 없으면 완전히 쉰다.
...
| 등급 | 판정 | 후속 행동 |
|---|---|---|
| A | 35분 이내 무힌트 해결 | 같은 문제 재코딩 없음; 신호·불변식 한 줄 기록 |
| B | 55분 이내 무힌트 해결 | 다음 날 시작 전 5분 구두 회상 |
| C | H1/H2 후 독립 구현 | 48시간 안에 코드 없이 상태·불변식·의사코드 20분 복원 |
| D | H3/코드 확인 후 핵심 로직 복원 | 가장 가까운 일요일 버퍼에서 빈 화면 40분 한 번 |
| E | 설명을 본 뒤에도 핵심 로직 복원 실패 | 개념 20분 후 버퍼에서 40분 한 번; 실패 시 더 쉬운 형제 문제 |
...
| 날짜 | 문제 | 결과 | 시간 | 힌트/API | 등급·단계 | 수정 계획 후속 행동 |
|---|---|---:|---:|---|---|---|
| 8/2 | 할인 행사 (131127) | 통과 | 15분 | H0, 검색 없음 | A | ~~D7 8/9~~ → 없음 |
| 8/2 | 기능개발 (42586) | D0 통과 | 8분 | 코드 확인 | D | 8/3 재구현 완료; 추가 없음 |
| 8/3 | 기능개발 (42586) | D1 통과 | 7분 | H0, J | 구 계획 D1 성공 | ~~D3 8/5, D7 8/9~~ → 취소 |
| 8/3 | 정수 삼각형 (43105) | D0 통과 | 7분 | H3 | D | 8/4 재구현 완료; 추가 없음 |
| 8/3 | 완주하지 못한 선수 (42576), HashMap | 통과 | 10분 | H0, J | 방식 1 성공 | - |
| 8/3 | 완주하지 못한 선수 (42576), 정렬 | 통과 | 10분 | H0, 검색 없음 | 방식 2 성공 | - |
| 8/3 | 폰켓몬 (1845) | 통과 | 8분 | H0, 검색 없음 | A | ~~D7 8/10~~ → 없음 |
| 8/4 | JadenCase 문자열 만들기 (12951) | D0 통과 | 최초 11분, D0 4분 | H0, J | 알고리즘 A / Java 도구 복원 성공 | ~~StringBuilder D1 8/5~~ → 취소 |
| 8/4 | 정수 삼각형 (43105) | D1 통과 | 7분 | H0, 검색 없음 | 구현·설명·경계 테스트 성공 | ~~D3 8/6, D7 8/10~~ → 취소 |
| 8/4 | 전화번호 목록 (42577) | D0·정렬 비교 통과 | D0 5분, 정렬 10분 | H3, H2, J(substring) | D | 이미 핵심 로직 복원; 고정 일정 취소 |
| 8/4 | 의상 (42578) | 통과 | 6분 | H0, 검색 없음 | A | ~~D7 8/11~~ → 없음 |
| 8/5 | 주식가격 (42584), O(n²) | 통과 | 30분 | H0, 검색 없음 | A | O(n) 단조 스택 비교 예정 |
...
검사할 번호 쌍:
정렬 후 서로 인접한 phone_book[i]와 phone_book[i+1]

이유:
한 번호를 접두어로 갖는 문자열들은 사전순 정렬에서 연속으로 모인다.
접두어 관계가 하나라도 있다면 인접한 쌍 중에서도 발견된다.

불변식:
i번째 반복 전에 앞에서 확인한 모든 인접 쌍에는 접두어 관계가 없다.
...
시간 / 공간 복잡도:
정렬 O(n log n × L), 인접 검사 O(n × L)
전체 O(n log n × L), Java 객체 배열 정렬 보조 공간 최악 O(n)
```

`substring(...).equals(...)` 대신 `startsWith(...)`를 사용할 수도 있지만 현재 코드도 정확하다. 입력에는 같은 번호가 중복되지 않으므로 길이가 같은 서로 다른 번호는 접두어 관계가 아니다.

### 전화번호 목록 최종 상태

```text
HashSet 방식: H3 후 D0 성공
정렬 방식: H2 후 구현 성공
최종 문제 등급: D
D1: 8/5
D3: 8/7
...
## 8/4 첫 번째 계획 분량 완료 (수정 전 운영 기록)

오늘 예정된 복습과 핵심 문제를 완료했다.
...
1. 전화번호 목록 D1
2. 기능개발 D3
3. StringBuilder D1 도구 회수
```

~~사용자 요청에 따라 날짜를 따라잡을 때까지 하루 2일치를 진행한다.~~

2026-08-04 수정 계획에서 이 운영 방식은 폐기되었다. 아래 의상까지는 실제 수행 기록으로 보존하되 8/1~8/3을 소급 완료한 것으로 세지 않는다.
...
```text
상태 / 자료구조:
key는 의상 종류, value는 그 종류에 속한 의상 개수인 HashMap

한 단계마다 바뀌는 것:
의상을 읽을 때 해당 종류의 개수를 1 증가시킨다.
종류를 처리할 때 현재까지의 조합 수에 (그 종류의 개수 + 1)을 곱한다.

불변식:
종류를 k개 처리한 뒤 answer는 처리한 각 종류에서 하나를 입거나 입지 않는 모든 선택 조합의 수다.

종료 조건:
모든 종류의 선택 수를 곱한 뒤 아무것도 입지 않은 한 가지 조합을 제외한다.
...
`HashMap<String, Integer>` 대신 변수 타입을 `Map<String, Integer>`로 선언할 수도 있지만 현재 코드도 정확하다.
...
> 그냥 경우의 수 계산이라. 정확히는, 아무것도 선택하지 않는 케이스 제외

정리: 각 종류의 `+1`은 그 종류에서 아무것도 고르지 않는 선택이다. 모든 종류의 선택 수를 곱하면 아무 의상도 입지 않는 조합까지 포함되므로 마지막에 `1`을 뺀다.

판정: 조합식의 의미를 설명했으므로 A 확정. ~~D7 8/11~~ → 수정 계획에서 후속 재코딩 없음.

당시 계획한 따라잡기 두 번째 묶음까지 완료한 기록이며, 수정 계획에서는 7/31 실제 수행까지로 분류한다.

---

## 10. 2026-08-04 수정 계획 동기화

```text
7/28~7/31: 실제 수행 기록 보존
8/1~8/3: 미진행, 소급 금지
8/4: 다리를 지나는 트럭 미진행; 다음 날 합산 금지
8/5: 주식가격 (42584)
기본 신규량: 실제 날짜에 배정된 한 문제
고정 D1/D3/D7: 폐기
```
...
목표:
~~가격 하강 시간에 대한 리스트 반환~~
→ 각 시점의 가격이 처음으로 떨어질 때까지 걸린 초 수를 int[]로 반환한다.

순서 / 중복 / 연속성:
~~순서, 연속성은 중요~~
→ 시간 순서와 연속성은 중요하다. 같은 가격은 하락이 아니므로 중복·동일 값도 중요하다.

최대 입력과 O(n²) 예상 연산량:
~~O(n²)~~
→ n은 최대 100,000. 최악에는 n(n-1)/2, 약 50억 번 비교할 수 있다.

단순 풀이:
각 시작 위치에서 뒤의 가격을 전부 비교하되 처음 낮은 가격을 발견하면 중단한다.

상태 / 자료구조:
~~array~~
→ 반환 배열 answer, 기준 시점 i, 기준으로부터 지난 시간 j.

한 단계마다 바뀌는 것:
~~숫자 크기~~
→ prices[i]와 prices[i+j]를 비교하며 j를 증가시키고, 처음 낮아지는 순간 answer[i]를 j로 확정한다.

불변식:
내부 반복에서 j를 검사하기 전까지 i+1부터 i+j-1의 가격은 모두 prices[i] 이상이다.

종료 조건과 반환값:
더 낮은 가격을 만나면 answer[i]=j로 확정하고 break한다. 끝까지 만나지 못하면 마지막 시점까지의 시간을 유지한다.

시간 / 공간 복잡도:
~~O(n²) / O(n)~~
→ 최악 시간 O(n²). 반환 배열은 O(n), 반환값을 제외한 추가 공간은 O(1).

확인할 예외:
~~맨 마지막은 무조건 0~~
→ 마지막 원소는 0 / 끝까지 떨어지지 않음 / 바로 다음 초에 떨어짐 / 같은 가격이 연속됨.
```

### 통과해도 O(n²)인 이유

`break`는 평균 실행량을 줄일 뿐 최악 상한을 바꾸지 않는다. 가격이 바로 떨어지는 입력은 각 i에서 빠르게 끝나지만, 가격이 계속 오르거나 같으면 각 i가 배열 끝까지 확인하므로 비교 횟수는 n(n-1)/2이다. 이번 통과는 채점 데이터와 실제 실행 조건에서 허용됐다는 뜻이며, 최악 입력에서도 O(n)이라는 뜻은 아니다.

### 오늘의 2차 학습

같은 문제를 인덱스 단조 스택으로 O(n)에 해결하고 두 방식의 연산량을 비교한다. 1차 풀이는 30분·H0·검색 없음으로 A이며, O(n) 학습은 등급을 낮추지 않는다.
